...
public class MyThreeWayBTree implements NavigableSet<Integer> {
	...
	public boolean remove(Object o) {
		...
		MyThreeWayBTreeNode ptN = findNodeToRemove(o); //삭제할노드
		if (ptN == null) return false;

		if (ptN.getChildrenList().isEmpty()) { //리프노드이면
			removeKeyFromNode(ptN, o);
		} else {
	...
	private MyThreeWayBTreeNode findNodeToRemove(Object o) {
		MyThreeWayBTreeNode ptN = root;
		Number key = (Number) o;

		while (true) { //삭제할노드검색
			int i = 0;
			for (Integer val : ptN.getKeyList()) {
				if (key.intValue() < val) break;
				else if (val == key.intValue()) return ptN;
				else i++;
			}
			if (ptN.getChildrenList().isEmpty()) break;
			else ptN = ptN.getChildrenList().get(i);
		}
		return null;
	}

	private void removeKeyFromNode(MyThreeWayBTreeNode node, Object o) {
